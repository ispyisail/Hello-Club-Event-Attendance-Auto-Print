...
> **In-flight plan:** `docs/PLAN-cli-tag-conversion.md` describes an agreed but not-yet-implemented conversion to a pure CLI driven by `print:` tags in event descriptions (removes the web dashboard and `categories` config). Check whether that work has landed before relying on dashboard/categories details below.
...
```bash
npm test                    # Run Jest tests (181 tests, 10 suites)
...
docs/                       Full documentation set — see docs/INDEX.md; ARCHITECTURE.md, CONFIGURATION.md, WEB-DASHBOARD.md, DEPLOYMENT.md, TROUBLESHOOTING.md are the deep dives
.claude/rules/              Longer-form pattern references (API client, database, service, testing, web dashboard, code style) — copy-pasteable snippets beyond the summary below
```
...
- To add a migration: create `src/core/migrations/003_description.js` exporting `{ up, down }` — picked up automatically by filename sort order. ESLint ignores the `migrations/` directory (see `.eslintignore`).
...
- Winston logger only — no raw `console.log` in `src/`; it is tolerated in `web-dashboard/` (ESLint `no-console` is off there).
